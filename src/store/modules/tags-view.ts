...
class TagsView extends VuexModule implements ITagsViewState {
  ...
  private ADD_VISITED_VIEW(view: ITagView) {
    ...
    this.visitedViews.push({ ...view, title: (view as any).meta.title || 'no-name' })
  }
  ...
  private DEL_OTHERS_VISITED_VIEWS(view: ITagView) {
    this.visitedViews = this.visitedViews.filter(v => (v as any).meta.affix || v.path === view.path)
  }
  ...
  private DEL_ALL_VISITED_VIEWS() {
    // keep affix tags
    const affixTags = this.visitedViews.filter(tag => (tag as any).meta.affix)
